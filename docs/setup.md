...
Após atualizar para a restauração de fábrica 0.3.5, execute `alembic upgrade head` com a credencial administrativa: a revisão `5e92acb01877` concede `INSERT` em `device_claims` ao papel da API, necessário para criar o QR novo na confirmação do ESP32. Se a rede local bloquear a porta PostgreSQL, um túnel SSH pode ser usado com `CHARGEGRID_MIGRATION_HOSTADDR` e `CHARGEGRID_MIGRATION_PORT`; o host original da URL continua sendo validado pelo TLS. Nunca coloque `MIGRATION_DATABASE_URL` na Vercel.
