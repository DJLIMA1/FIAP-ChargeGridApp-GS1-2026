...
# Justificativas e relação curricular:

O Objetivo que a GoodWe nos propôs implicava a criação de um aplicativo que solucionasse a deficiência transacional que eles possuíam com relação á eletrospostos. O objetivo como descrito acima é buscar aumentar esse mercado, propondo facilidade e consequentemente suprindo a necessadiade da goodwe, os ajudando econocimante também por meio de taxas sobre as transações realizadas. Para atingir esse objetivo optamos por seguir a rota da beleza e facilidade do aplicativo, permitindo atrair mais usuários para ele, com isso optamos pelo Flet por ser facilmente utilizável para aplicações em python para APK, e com uma estrutura de dados simples porém robusta como ensinado nas matérias de Python e Data Strucutures. O aplicativo já é capaz de simular de forma extremamente simples uma transação e seus dados, comprovando que a beleza e usabilidade que são os pilares do projeto foram alcançadas. Para criação desse projeto devemos agradecer nossos professores, principalmente de Data Structures, que nos ajudou a observar a otimização de dados, Python que serviu como base para nossa programação e computer organization e Science que serviram como base pra comerçarmos a implementar as soluções físicas.
